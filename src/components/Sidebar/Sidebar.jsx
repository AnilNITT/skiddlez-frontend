...
const Sidebar = React.memo(({ currentModal,onUsers, onUserSettings, onLogout, user,setDefaultUI, setDarkUI, setLightUI, theme, sidebar, expand, collapse, history, location}) => {
    ...
    return (
        <div className = {classNames('sidebar', sidebar ? "" : "collapse")}>
            <ul>
                <li>
                    <button className={currentTab === 0 ? "active-sidebar-link" : "btn"} type = "button" onClick = {() => handleNavigation(0)}>
                    <i className="fas fa-chart-line clr"/><p>Activity</p>
                    </button>
                </li>
                <li>
                    <button className={currentTab === 1 ? "active-sidebar-link" : "btn"} type = "button" onClick = {() => handleNavigation(1)}>
                    <i className="fas fa-th-list"/><p>Projects</p>
                    </button>
                </li>
                <li>
                    <button className={currentTab === 2 ? "active-sidebar-link" : "btn"} type = "button" onClick = {() => handleNavigation(2)}>
                    <i className="fas fa-clipboard-list"/><p>Board</p>
                    </button>
                </li>
                <li>
                    <button className={currentTab === 3 ? "active-sidebar-link" : "btn"} onClick={() => handleNavigation(3)} type="button">
                    <i className="fas fa-users"/><p>Members</p>
                    </button>
                    </li>
                    <li>
                    <button className={currentTab === 4 ? "active-sidebar-link" : "btn"} onClick={() => handleNavigation(4)} type="button">
                    <i className="fa fa-cog"/><p>Settings</p>
                    </button>
                    </li>
                <li>
                    <UserPopup id = 'user-modal-sidebar' onSettings = {onUserSettings} user = {user} mode = {theme} setDefaultUI={setDefaultUI} setDarkUI = {setDarkUI} setLightUI = {setLightUI} onLogout = {onLogout}>
                        <button type = 'button'>
